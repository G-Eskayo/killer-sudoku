...
- **Given**: a digit pre-filled by the puzzle before the player starts, exactly as in standard
  Sudoku. A given is an ordinary member of its normal 2-4 cell cage — the cage's size, sum, and
  border are unaffected, one of its cells just already shows its digit. See [[0008]] for why the
  puzzle has givens at all and why they're not single-cell cages ([[0006]], superseded), and
  [[0009]] for why there's only ever this one given mechanic now.

## Puzzle generation

- 100% of the grid is covered by cages sized 2-4 — no single-cell cages, and (since [[0009]]) no
  cells excluded from the cage system either. A given-count is drawn from a per-tier range
  ([[0008]]) and revealed inside whatever cage those cells already belong to. Given-density *is*
  the difficulty lever (sparse at Expert, dense at Easy) — see Difficulty below. There used to be
  a second, separate hybrid mode (cage-exempt givens, for a Beginner tier) — removed in [[0009]]
  once given-density alone already covered that easier-onboarding role.

## Difficulty

- **Difficulty tier**: one of Easy, Medium, Hard, Expert — four tiers total, Expert is the single
  top tier ([[0005]]). A fifth tier (Beginner, hybrid mode) existed until [[0009]] removed it.
- **Given-density difficulty signal**: the active difficulty signal ([[0008]]) — how many of the
  81 cells are revealed as givens, scaled by tier (Expert sparsest, Easy densest). Decided
  directly at generation time, not discovered by retrying and re-checking. Supersedes two earlier
  attempts: solving-technique simulation ([[0003]]) and solver search effort ([[0007]]), neither
  of which is used for grading anymore — see [[0008]] for why.
- **Solving-technique grading**: originally meant determining which solving techniques a puzzle
  *requires* (basic cage-sum arithmetic and single candidates for low tiers, advanced cage-
  ...
  size — [[0003]]. In practice this never differentiated real generated puzzles at all (every one
  graded hardest-tier regardless of actual difficulty). The technique simulator (`DifficultyGrader`)
  itself is still real, tested code; it's just never been what the New Puzzle flow actually uses
  for grading.
