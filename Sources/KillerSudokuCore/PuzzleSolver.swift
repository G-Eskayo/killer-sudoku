...
private final class SolverState {
    ...
    /// Which digits (as a bitmask) are still legal for the given cell within its own cage. See
    /// [[CageConstraint]] — this runs at every node of the outer search, so per-call allocation
    /// cost matters a lot here; that's why it stays a thin wrapper around the shared, allocation-
    /// free logic rather than, say, building an intermediate array.
